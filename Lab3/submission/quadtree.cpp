...
void quadtree::clear(){
	//root = NULL;
	tree_size = 0;
	postorder_clear_helper(root);
	root = NULL;

	cout<<"success"<<endl;
}

void quadtree::postorder_clear_helper(struct Node* current_node){

	if (current_node != NULL) 
    	{ 
        	postorder_clear_helper(current_node -> NE); 
        	postorder_clear_helper(current_node -> NW); 
        	//cout<<current_node->Attribute.Name<<" "<<endl;
        	//cout<<regex_replace(current_node->Attribute.Name, regex("^\\s++"), string(""))<<" "; 
        	postorder_clear_helper(current_node -> SW); 
        	postorder_clear_helper(current_node -> SE);
        	current_node = NULL;
    	} 
}
